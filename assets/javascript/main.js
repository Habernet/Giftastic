...
games = [
    "Halo: Combat Evolved", "Fortnite", "PUBG", "Apex Legends", "Call of Duty"
]
...
$("#buttons").on("click",".btn", function () {
    ...
    var queryURL = "https://api.giphy.com/v1/gifs/search?q=" + searchFor + "&limit=100" + "&api_key=F0y8OeTPpYSZkVLz2fLvNXdxqtpfpPSp";
    ...
    $.ajax({
        ...
        for (let i = 0; i < 10; i++) {
            ...
            var gifDiv = $("<div>");
            var gif = $("<img>").attr("src", response.data[randomIndex].images.fixed_height_still.url).addClass("gif");
            gif.attr("data-state", "still");
            gif.attr("data-still", response.data[randomIndex].images.fixed_height_still.url);
            gif.attr("data-animate", response.data[randomIndex].images.fixed_height.url);
            var rating = $("<p>").text("Rating: " + response.data[randomIndex].rating);
